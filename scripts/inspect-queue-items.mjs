...
function tableInfo(name) {
  ...
  return rows.map(r => ({ cid: r.cid, name: r.name, type: r.type, notnull: r.notnull, dflt_value: r.dflt_value, pk: r.pk }));
}

function indexes(name) {
  const rows = db.prepare(`PRAGMA index_list(${name})`).all();
  return rows.map(r => ({ name: r.name, unique: r.unique, origin: r.origin, partial: r.partial }));
}
...
// If previous 007 renamed it partially, show queue_items_old too (if exists)
